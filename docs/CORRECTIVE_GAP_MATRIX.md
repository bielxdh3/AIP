...
| 8 | Local audio input, STT, TTS, wake word, custom voice | Architecture checkpoint with synthetic fixtures | Rust/SQLite metadata, fixture adapters, controls, and text fallback; no microphone/model/audio output | Windows device enumeration, bounded recording, replaceable local STT/TTS, wake-word loop, custom voice adapter/runtime | Existing migration/module/contract/UI fixture tests | Mic/speaker quality, wake-word behavior, voice naturalness | FOUNDATION ONLY |
| 9 | Safe supervised local filesystem and provider-neutral tools | Local mock checkpoint | Fixture-scoped preview/approval/audit paths; no host effect | Root-scoped real list/create/move/copy/organize/reversible-delete/read plus bounded adapter harnesses | Existing mock, approval, parser, and gate tests | Confirmation UX and credential-bound live providers | FOUNDATION ONLY |

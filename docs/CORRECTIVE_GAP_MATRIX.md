...
| 12 | Android project, reproducible APK, pairing, authenticated desktop transport, chat/history/queue/notifications | Metadata-only companion checkpoint; no APK/transport | Functional Android APK/client-local protocol slice; no desktop transport | Desktop authenticated transport integration, physical-device validation | Android JVM tests, lint, assembleDebug, CI artifact | Physical-device UX and permission behavior | FUNCTIONAL — TRANSPORT NEXT |

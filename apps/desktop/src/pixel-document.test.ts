...
describe("pixel documents", () => {
  it("normalizes layers and updates only the selected layer", () => {
    ...
    expect(
      updatePixelLayer(document, "hair", (layer) => ({
        ...layer,
        visible: true,
      })),
    ).toMatchObject({
      layers: [
        ...
        { id: "hair", visible: true, locked: true },
      ],
    });
  });
...
it("rasterizes every cell crossed by a fast stroke", () => {
  expect(rasterLine(1, 1, 5, 3)).toEqual([
    [1, 1],
    [2, 2],
    [3, 2],
    [4, 3],
...
describe("pixel flood fill", () => {
  ...
  it("keeps boundaries, identical colors, locked layers, and other layers untouched", () => {
    const layer = {
      ...
      visible: true,
      locked: false,
      pixels: [[1, 1, "#abc"]] as [number, number, string][],
    };
    expect(floodFillLayer(layer, 1, 1, "#abc")).toBe(layer);
    expect(
      floodFillLayer({ ...layer, locked: true }, 1, 1, "#def"),
    ).toMatchObject({ pixels: [[1, 1, "#abc"]] });

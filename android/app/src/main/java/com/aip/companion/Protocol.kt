...
object FrameCodec {
    ...
    fun encode(f:Frame):String { if(f.protocol!=PROTOCOL||f.kind.isEmpty()||f.kind.length>MAX_TEXT||f.clientId.isEmpty()||f.clientId.length>512||f.nonce.isEmpty()||f.nonce.length>128||f.counter<0||f.payload.toByteArray().size>MAX_PAYLOAD||f.mac.length!=64)throw ProtocolError.Fields; val s="{\"protocol\":\"${esc(f.protocol)}\",\"kind\":\"${esc(f.kind)}\",\"clientId\":\"${esc(f.clientId)}\",\"sessionId\":${f.sessionId?.let{"\"${esc(it)}\""}?:"null"},\"nonce\":\"${esc(f.nonce)}\",\"counter\":${f.counter},\"payload\":\"${esc(f.payload)}\",\"mac\":\"${f.mac}\"}"; if(s.toByteArray().size+1>MAX_FRAME)throw ProtocolError.Oversized; return "$s\n" }
...
class CompanionClient(private val transport:SocketTransport,private val protocol:CompanionProtocol){fun request(kind:String,payload:String):ClientResult<Frame>{return try{val response=transport.request(protocol.frame(kind,payload))?:return ClientResult.Offline();ClientResult.Online(protocol.accept(response))}catch(e:ProtocolError){throw e}catch(_:Exception){ClientResult.Offline()}};fun pair(payload:String)=request("pair",payload);fun session(payload:String)=request("session",payload);fun reconnect(payload:String)=request("reconnect",payload);fun history(payload:String)=request("history",payload);fun queuePreview(payload:String)=request("queue_preview",payload);fun queueApprove(payload:String)=request("queue_approve",payload);fun queueCancel(payload:String)=request("queue_cancel",payload);fun queueRetry(payload:String)=request("queue_retry",payload);fun revoke(){protocol.revoke()};fun rotate(key:ByteArray){protocol.rotate(key)}}
